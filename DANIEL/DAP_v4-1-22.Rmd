...
* **Disgust**: a Likert scale (1 to 7) of the subjects’ emotional subjective feeling of disgust while juding the word pairs.
...
This figure shows the distribution of the subjective evaluation of disgust across all pair of words, depending of the emotion elicited by the words. As one can see, there still is a floor effect, but it seems less strong for the disgust emotion, as one can expect. Indeed, when the word pairs are eliciting disgust, we expect people to categorize it as such. However, eliciting emotions through words, and specifically disgust, is difficult and depends on the sensitivity of each individual and its perceived intensity. Plus, studies have shown that the emotion of disgust is often misidentified (Yoder et al., 2016). Another representation including the gender can be found in Appendix 1. 
...
(a) a simple logistic ordinal regression, (b) a simple mixed-effect model only considering the subjects' repetition as a random effect, and (c) a more complete mixed-effect model with random slopes allowing fixed effects to vary for each emotion, in addition to the previous random effect.
...
As shown in this table, we can conclude that the model that seems to fit the best our data is the full random structure model, mixed-effect model with random slopes allowing fixed effects to vary for each emotion, in addition to the subjects' random effect, as it has the smaller AIC, and more equivalent degrees of freedom.
<br>

We can explicitly test the effect of the random slope allowing fixed effects to vary for each emotion to be sure it's useful to bring complexity into the model.

```{r random slope, cache=TRUE, message=FALSE, warning=FALSE}
anova(m2.2, m3.2)
```
Since the test is significative, we can conclude that it is important to include the random slope in the best model. 
<br>
...
```
As shown in the summary above, the χ² test tells us that the simplest model, without gender, is the best model (as none is significative). In addition with the comparison of the AIC estimates, we can say that the most parsimonious model is the model without the gender effect. 
