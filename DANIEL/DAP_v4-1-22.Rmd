...
pacman::p_load(here, ggplot2, ordinal, ggfortify, GGally, plyr, sjPlot, likert, vtable, brant, Hmisc, MASS, kableExtra, gtsummary)
...
#data scanning 
Raw_data <- read.csv2("Raw_data2.csv")
=======
#load data 
Raw_data <- read.csv2("Raw_data.csv")
>>>>>>> fd3180a (comments Daniel)
```
...
<!-- use `<br>` to put brealine in your parapgraph so it is not suffocating -->
<br>
...
In this project, we are going to use part of this dataset to analyse and compare three statistical methods. We will compare (a) a simple logistic ordinal regression, (b) a simple mixed-effect model only considering the subjects' repetition
<!-- as a random effect -->
<!-- fait gaffe a la diff between random effect (ID) et random slope (emotion | ..) -->
, and (c) a more complete mixed-effect model 
 <!-- with random slopes allowing fixed effects to vary for each emotion. -->
 <!-- We follow the 'Keep it maximal' procedure from Barr et al. (2103) -->
to find a model that best fits our data, considering how the emotion elicited in the word pairs and the gender impact the subjective ratings of disgust by the participants.
...
<!-- explain why (sensitivity of the question etc..) -->
Another representation including the gender can be found in Appendix 1. 



## Modeling

Even if we have seen in our exploration that there seems to be a floor effect, we are going to do a diagnostic of a linear model to certify the inadaptation of this model to analyse our data.
...
<!-- change that accordingly: see above -->
(a) a simple logistic ordinal regression, (b) a mixed-effect model considering the subjects' repetition, and (c) a mixed-effect model considering the subjects' repetition and the random effect of the emotions. 


<!-- cahce = TRUE so you dont have to recompute everything each time! -->
```{r, cache=TRUE}
...
As shown in this table, we can conclude that the model that seems to fit the best our data is the full random structure model, 
<!-- chnage here -->
the mixed-effect model considering the subjects' repetition and the random effect of the emotions, as it has the smaller AIC, and more equivalent degrees of freedom.

```{r random slope, cache=TRUE, message=FALSE, warning=FALSE}
anova(m2.2, m3.2)
```
<!-- here we explicitly test the random slope, you could add a samll comment to say that the random slope is significant and thus it is important to but in the model or something -->
...
anova(m3.0, m3.1, m3.2, test='Chisq') #can replace all the above by that and you still have AIC and DF, but also Pval, THIS show that m3.0 is the best AIC and that the other one are not better (i.e. not significant pval)
...
# rownames(c4) <- c("Model with interaction", "Model without interaction","Model without gender effect")
# colnames(c4) <- c("Degrees of freedom", "Akaike Information Criterion")
# c4
```
As shown in the summary above, and following the AIC estimate, the most parsimonious model is the model without gender effect. 
...
<!-- or something like that because everything we have done WAS analyzing it-->
...
```{r}
# coef(summary(m3.0)) #these  two same the same thing
sjPlot::tab_model(m3.0, show.stat = TRUE, show.aic = T, show.reflvl = T, show.est = T, p.style = "stars")
```

<!-- you shouldd change this sentence.. not scientific at all -->
That means that the word pairs' emotion explains the disgust evaluation (p< 0.001). 
<!-- see the https://github.com/munoztd0/DAP_2021/discussions/9 for a guide ona hwo to report results -->
...
<!-- effect size, it it right but explained variance here is more precise -->
...
```{r}
# mult<-Data[,3:4]
# counts <- ddply(mult, .(mult$emotion, mult$gender), nrow)
# names(counts) <- c("emotion", "gender", "Freq")
# counts
# counts2<-reshape(counts, idvar = "emotion", timevar = "gender", direction = "wide")
# counts2
#x2<-chisq.test(counts2) #don't know why it doesn't work here


 # replacded by that so you have all the percents AND the CHI squared test!
Data[c("emotion", "gender", "disgust")] %>%  tbl_summary(by=emotion)  %>% modify_caption("**Summary statistics by emotion**") %>%
  add_p() %>% bold_labels()

```
<!-- check here what it means and chang accordingly -->
We must check for any collinearity between our predictors. Our final model only has the wordpairs' emotion as predictor, so there is no possible collinearity with other predictors. For the other models, we also included the gender.
...
<!-- check here terms -->
For that, the mixed-effect model considering the subjects' repetition and the random effects not including the gender was the best model tested. 
<!-- check here inference -->
In other words, gender doesn't seem significative in the disgust evaluation, but it seems important to control for the way each subject do the evaluation. Further investigation can be made with the fear evaluation of this dataset to support or reject this proposition.
<!-- details to gain some point -->
<!-- add a general world or two about mixed models vs simple linear -->
 <!-- add a general world or two about  ordinal logistic regression, how is it interpretable, why it difers from comparing means and why it is suited for this kind of data..-->
...
ggplot(Data, aes(emotion,disgust,colour=gender))+geom_jitter(alpha=0.1)+
...
<!-- 4. Hidden code Remove that and put the code inline but hiden on toggle -->
<!-- ------------------ -->
