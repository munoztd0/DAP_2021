...
The effect of the emotion predictors of fear (OR = 0.13, 95\% CI
0.10-0.16, p \textless{} 0.001) and neutral (OR = 0.02, 95\% CI
0.01-0.02, p \textless{} 0.001) are different from disgust. The random
slope for emotions' fixed effects has been reported above.
...
\hypertarget{residual-representation}{%
\paragraph{Residual representation}\label{residual-representation}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Residual{-}vs{-}covariate plot and Q{-}Q plot}
...
\NormalTok{pres }\OtherTok{\textless{}{-}} \FunctionTok{presid}\NormalTok{(m1}\FloatTok{.3}\NormalTok{)}

\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Data}\SpecialCharTok{$}\NormalTok{disgust, }\AttributeTok{y =}\NormalTok{ pres), }\FunctionTok{aes}\NormalTok{(x, y)) }\SpecialCharTok{+}
\FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{color =} \StringTok{"\#444444"}\NormalTok{, }\AttributeTok{shape =} \DecValTok{19}\NormalTok{, }\AttributeTok{size =} \DecValTok{2}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}
\FunctionTok{ylab}\NormalTok{(}\StringTok{"Probability{-}scale residual"}\NormalTok{)}
\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{y =}\NormalTok{ pres), }\FunctionTok{aes}\NormalTok{(}\AttributeTok{sample =}\NormalTok{ y)) }\SpecialCharTok{+}
\FunctionTok{stat\_qq}\NormalTok{(}\AttributeTok{distribution =}\NormalTok{ qunif, }\AttributeTok{dparams =} \FunctionTok{list}\NormalTok{(}\AttributeTok{min =} \SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\AttributeTok{max =} \DecValTok{1}\NormalTok{), }\AttributeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}
\FunctionTok{xlab}\NormalTok{(}\StringTok{"Sample quantile"}\NormalTok{) }\SpecialCharTok{+}
\FunctionTok{ylab}\NormalTok{(}\StringTok{"Theoretical quantile"}\NormalTok{)}
\FunctionTok{grid.arrange}\NormalTok{(p1, p2, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{) }\CommentTok{\# Figure 1}
\end{Highlighting}
\end{Shaded}
