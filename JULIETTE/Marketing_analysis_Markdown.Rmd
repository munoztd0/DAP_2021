...
## III - Hypotheses 
<!-- comment: Here you should clean that up, what are you rhypthesis ? why these graphs are important or not ? what are they saying ? why are they pltted differenlty ?  -->
...
data %>% 
      ...
      ggplot(aes(AcceptedCmpTotal, Income)) +  geom_violin(trim=FALSE, fill='#A4A4A4', color  ="darkred") # geom_boxplot(width=0.05, outlier.colour =  "red") + theme_minimal()
...
```
<!-- comment: Here why is NumStorePurchases useful ? based on the plot or not ? if Not, why the plot ? -->
<br>
...
<!--  PUT THAT INTO ANNEX, THIS IS NOT FUNDAMENTAL INFORMATION + YOU ALREADY HAVE THE SAME PLOT JUST ABOVE-->
...
### A.1. Model creating and assumptions
<br>
```{r, out.width="50%", warning=F, message=F}
m1 <- lm(data=data, NumStorePurchases ~ Kidhome*Income*Education*age) 
plot(m1, c(1:2,4), ask=F)
```
<!-- comments: Here put it in annex-->
<!-- add comments -->
<br>
<br>
Looking at the QQplot, we don't see any specific patterns appearing, which means the residuals are distributed normally. 
<!-- comments: Not really that neither-->
Although we do observe smaller ends for the lowest theoritical quantiles.
<br>
<br>
...
<!-- because they probably did not write the true value of their income. comment: Mhh that's very speculative why not the opposite ? -->
...
<!-- comment: Here put all of this in annex too -->
...
<br>
<br>
Paying attention to the QQplot, we see that extreme points are more normal. 
<!-- comment: that's not that, lets talk about it or see this https://moodle.unige.ch/mod/page/view.php?id=504355  -->
The Cook's distance graph also doesn't show values higher than 0,5.
...
<!-- what is effect size and why use it ? -->
```{r}
...
### **A.6. Plots and summary estimates** {.tabset}
...
<!-- comment: Why, what doies it meant ? -->
