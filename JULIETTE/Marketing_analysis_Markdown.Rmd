...
Here we observe peculiar outliers for "Income". One income is equal to 666'666.
...
The aim of this analysis is to be able to predict the number of store purchases 
### A. Linear model
...
<!-- comment: Why, what does it meant ? -->
...
Annex
<br>
The number of store purchases seems to be the most useful variable to analyze since we want to determine which profiles buy the most in the store. In order to consider NumStorePurchases as a response variables for a linear model, we first have to check normality. 
<br>
