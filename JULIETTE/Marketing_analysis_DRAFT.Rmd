 ...
 *to predict the customer's behavior (Number of purchases in the store) depending on the most significant personal attributes*
...
NumStorePurchases seems to be the most useful variable to analyse since marketing analysis might want to determine which profils buy the most in the store. 
<!-- In order to consider NumStorePurchases as a response variables for a linear model, we first have to check normality. Here, skewness appears to be 0,7 which is at an acceptable level. PUT THAT INTO ANNEX, THIS IS NOT FUNDAMENTAL INFORMATION + YOU ALREADY HAVE THE SAME PLOT JUST ABOVE-->
```{r}
...
<!-- comment: Here put all of this in annex -->
### **A.2. Assessing outliers :** 

Here we observe peculiar outliers for "Income". One income is equal to 666 666, and when income is higher than 150 000, people probably don't respond. We can safely remove these outliers.
```{r}
newdf = data %>%
